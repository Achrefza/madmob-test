  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mx-auto grid max-w-6xl grid-cols-1 gap-16 items-center lg:grid-cols-2">
        <div>
          <p className="text-xs uppercase tracking-[0.35em] text-[var(--accent-red)]">Interlude</p>
          ...
          <div className="relative overflow-hidden rounded-lg h-[420px] border border-white/10">
            ...
            <div className="absolute bottom-4 right-4 z-10 flex gap-2">
              <button
                ...
                className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-[var(--accent-red)] bg-black/60 text-[var(--accent-red)] backdrop-blur transition-all duration-300 hover:bg-[var(--accent-red)]/20"
            ...
            {slides.map((slide, index) => (
              <span
                ...
                className={`h-2 w-2 rounded-full transition-all duration-300 ${
                  index === currentSlide ? "bg-[var(--accent-red)]" : "bg-zinc-600"
                }`}
                aria-hidden="true"
              />
            ))}
